...
# - runfiles_current_repository() under a POSIX shell requires the caller's
#   script path as its first argument (there is no POSIX equivalent of
#   BASH_SOURCE). Under bash, the same 0-arg / numeric-index calling
#   convention as runfiles.bash is preserved.
# - rlocation() without a second argument auto-detects the caller's
#   repository via BASH_SOURCE only when running under bash. Under a POSIX
#   shell it defaults to the main repository; portable callers should pass
#   the source repo name explicitly.
...
# manifest scanning without lowercasing the entire line per iteration —
# lookups on large manifests were minutes-slow when a scan involved
# lowercasing every line character-by-character.
...
__runfiles_gsub() {
  ...
  _rf_gs_old="$2"
  _rf_gs_new="$3"
  # An empty needle would loop forever (case '*""*)' matches unconditionally
  # and the parameter-expansion strips nothing). No current caller passes an
  # empty needle; guard as a footgun for future callers.
...
# Returns the input unchanged when there is no non-safe-followed-by-safe pair
...
__runfiles_compute_repo_prefix() {
  ...
  printf '%s*%s' "$_rf_cp_head" "$_rf_cp_suffix"
}
...
__runfiles_find_repo_mapping() {
  ...
  if [ -n "$_RLOCATION_CASE_INSENSITIVE" ]; then
    ...
    while IFS= read -r _rf_rm_line || [ -n "$_rf_rm_line" ]; do
      # Compare only the prefix (src,tgt,) case-insensitively — cheap early
      # exit for the ~all lines that don't match, avoiding the full-line
      # O(N^2) lowercase that dominated large repo-mapping scans on Windows.
  ...
  if [ -f "${RUNFILES_REPO_MAPPING:-}" ]; then
    ...
    if [ -n "$remainder" ]; then
      ...
      if [ -n "${2+x}" ]; then
        ...
        # actual caller — mirrors runfiles.bash. Under a POSIX shell the
        # auto-detection is impossible; documented in the header.
        # `|| true` preserves whatever repo name the parse-exec-path fallback
        # already printed to stdout, mirroring bash's `local -r x=$(...)` where
        # `local` masks the subshell's exit code.
